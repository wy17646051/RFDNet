...
class CenterPointBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        if iou is not None:
            ...
            iou = iou.view(batch, self.max_num, 1).squeeze(-1)
            iou = torch.clamp(iou, min=0, max=1)
            scores = torch.pow(scores, 1 - rectifier) * torch.pow(iou, rectifier)
...
@BBOX_CODERS.register_module()
class PolarCenterPointBBoxCoder(CenterPointBBoxCoder):
    def decode(self,
               heat,
               rot_sine,
               rot_cosine,
        ...
        heatmap_indice_real_x = reg.new_tensor(math.pi) - xs * self.out_size_factor * self.voxel_size[0]
        heatmap_indice_real_y = ys * self.out_size_factor * self.voxel_size[1] + 0.3
        site_x = torch.cos(heatmap_indice_real_x) * heatmap_indice_real_y
        site_y = torch.sin(heatmap_indice_real_x) * heatmap_indice_real_y
        if reg is not None:
            reg = self._transpose_and_gather_feat(reg, inds)
            reg = reg.view(batch, self.max_num, 2)
            xs = site_x + reg[:, :, 0:1]
        ...
        if rectify:
            rot = rot + torch.atan2(ys, xs)
            rot = rot - torch.floor(rot / (2* torch.pi) + 0.5) * (2* torch.pi)
        ...
        if vel is None:  # KITTI FORMAT
            ...
            if rectify:
                vx = vel[:, :, 0:1]
                vy = vel[:, :, 1:2]
                va = torch.atan2(vy, vx) + torch.atan2(ys, xs) 
                vr = torch.sqrt(vx * vx + vy * vy)
                vx = vr * torch.cos(va)
                vy = vr * torch.sin(va)
                vel = torch.cat([vx, vy], dim=2)
