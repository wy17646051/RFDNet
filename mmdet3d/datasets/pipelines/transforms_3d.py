...
@PIPELINES.register_module()
class NuScenesSematicObjectSample: 

    CONFIG = {
        'car':                  {'id_sem': 3, 'id_mix': [10, 11, 12, 13]},
        'truck':                {'id_sem': 9, 'id_mix': [10, 11, 12, 13]},
        'construction_vehicle': {'id_sem': 4, 'id_mix': [10, 11, 12, 13]},
        'bus':                  {'id_sem': 2, 'id_mix': [10, 11, 12, 13]},
        'trailer':              {'id_sem': 8, 'id_mix': [10, 11, 12, 13]},
        'barrier':              {'id_sem': 0, 'id_mix': [10, 11, 12, 13]},
        'motorcycle':           {'id_sem': 5, 'id_mix': [10, 11, 12, 13]},
        'bicycle':              {'id_sem': 1, 'id_mix': [10, 11, 12, 13]},
        'pedestrian':           {'id_sem': 6, 'id_mix': [10, 11, 12, 13]},
        'traffic_cone':         {'id_sem': 7, 'id_mix': [10, 11, 12, 13]},
    }

    def __init__(
        self, tulpe_info_path, data_root, 
        sample_cls, max_sample, min_sample, min_points, mix_range=[-50, -50, 50, 50], granularity=0.2, depth_allowance=1, 
        transforms=None, collision=False, mix_on_points=False, refresh_cache=False, info_path=None,
        points_loader=dict(type='LoadPointsFromFile', coord_type='LIDAR', load_dim=5, se_dim=[0, 1, 2, 3, 4]), pop_sem=True
    ):
        assert len(sample_cls) == len(max_sample) == len(min_sample) == len(min_points), \
            'Length of `sample_cls`, `max_sample`, `min_sample` and `min_points` should be the same.'
        assert all([i in self.CONFIG.keys() for i in sample_cls]), 'Sampling class not implemented.'
        x_min, y_min, x_max, y_max = mix_range
        assert x_max / granularity % 1 == 0 and x_min / granularity % 1 == 0 and \
               y_max / granularity % 1 == 0 and y_min / granularity % 1 == 0, 'Mix range not divisible by granularity.'
        assert all([i <= 50 and i >= 5 and i % 5 == 0 for i in min_points]), \
            '`min_points` needs to be a multiple of 5 between 5 and 50.'
        ...
        self.depth_allowance = depth_allowance

        self.transforms = transforms
        self.collision=collision
        self.mix_on_points=mix_on_points

        self.points_loader = mmcv.build_from_cfg(points_loader, PIPELINES)
        self.pop_sem = pop_sem

        x_min, y_min, x_max, y_max = self.mix_range
        r = lambda a, b: math.sqrt(a**2 + b**2)
        self.max_radius = min([r(x_min, y_min), r(x_max, y_min), r(x_min, y_max), r(x_max, y_max)])
        
        self.sem2det_map = torch.tensor([i for i in range(20)])
        for i, name in enumerate(self.sample_cls):
            self.sem2det_map[self.CONFIG[name]['id_sem']] = i
        self.mix_id_map = {
            v['id_sem']: torch.tensor([-1 if i in v['id_mix'] else i for i in range(20)]).long()
            for v in self.CONFIG.values()
        }
        
        depth_y, depth_x = torch.meshgrid([
            torch.arange(y_min / granularity, y_max / granularity), 
            torch.arange(x_min / granularity, x_max / granularity)
        ])
        self.depth_matrix = torch.sqrt((depth_y * granularity)**2 + (depth_x * granularity)**2)

        self.sample_matrix_buffer = {}

        if refresh_cache:
            with open(info_path, 'rb') as f:
                db_infos = pickle.load(f)
            tuple_info = self._cache_tuple_dbinfos(db_infos, tulpe_info_path)
        else:
            with open(tulpe_info_path, 'rb') as f:
                tuple_info = pickle.load(f)
        self.db_info, self.cache_info = tuple_info

    def _cache_tuple_dbinfos(self, db_infos, cache_path):
        infos_tuple = {}

        for cate in tqdm(self.sample_cls):
            cate_sem_id = self.CONFIG[cate]['id_sem']
            infos_tuple[cate_sem_id] = {}
            for n in range(0, 55, 5):
                infos_tuple[cate_sem_id][n] = []
                for info_cate in db_infos[cate]:
                    depth = np.linalg.norm(info_cate['box3d_lidar'][:2])
                    if info_cate['num_points_in_gt'] >= n and depth < self.max_radius:
                        infos_tuple[cate_sem_id][n].append(
                            (str(self.data_root / info_cate['path']), depth, info_cate['box3d_lidar']) 
                        )

        tuple_db_infos = (db_infos, infos_tuple)
        with open(cache_path, 'wb') as f:
            pickle.dump(tuple_db_infos, f)

        return tuple_db_infos
    
    def _sample_inst(self, sample_id, min_points):
        sample_list = self.cache_info[sample_id][min_points]

        if len(sample_list) == 0:
            sample_inst =  torch.zeros([0, 4], dtype=torch.float32)
            sample_depth = 0
            sample_bbox3d = sample_inst.new_zeros(9)
        else:
            sample_index = np.random.randint(0, len(sample_list))
            sample_path, sample_depth, sample_bbox3d = sample_list[sample_index]

            sample_bbox3d = torch.tensor(sample_bbox3d, dtype=torch.float32)
            sample_inst = self.points_loader(dict(pts_filename=sample_path))['points'].tensor
            # sample_bbox3d[:3] = sample_bbox3d.new_tensor([0, 0, sample_bbox3d[5] / 2])
            sample_bbox3d[:3] = sample_bbox3d.new_tensor([0, 0, 0])
    
            if self.transforms is not None:
                # TODO: add transforms
                pass

        return sample_inst, sample_depth, sample_bbox3d

    def _bev_filter(self, points):
        x_min, y_min, x_max, y_max = self.mix_range
        filter = ((points[:, 0] > x_min) & (points[:, 0] < x_max) &
                  (points[:, 1] > y_min) & (points[:, 1] < y_max))
        return filter
    
    def _flooding(self, matrix, radius):
        kernel = math.ceil((radius / self.granularity) * 2)
        if kernel % 2 == 0:
            kernel += 1

        padding = kernel // 2

        matrix = torch.max_pool2d(
            matrix[None, ...],
            kernel, 1, padding
        ).squeeze(0)
    ...
    def _sample_matrix(self, points_sem, label_sem, sample_id):
        x_min, y_min, x_max, y_max = self.mix_range
        h = int((y_max - y_min) / self.granularity)
        w = int((x_max - x_min) / self.granularity)

        sample_matrix = points_sem.new_zeros([h, w])

        mix_keep = self.mix_id_map[sample_id][label_sem] == -1

        mix_points = points_sem[mix_keep]
        mix_site_x, mix_site_y = mix_points[:, :2].unbind(-1)
        mix_index_x = ((mix_site_x - x_min) / self.granularity).floor().long().clamp(0, w - 1)
        mix_index_y = ((mix_site_y - y_min) / self.granularity).floor().long().clamp(0, h - 1)
        # TODO Modified to strict z value.
        sample_matrix[mix_index_y, mix_index_x] = mix_points[:, 2]

        return sample_matrix

    def _unsample_matrix_bbox(self, unsample_matrix, sample_bbox3d):
        # `unsample_matrix` will changed locally.

        x_min, y_min = self.mix_range[:2]
        x = (sample_bbox3d[0] - x_min) / self.granularity
        y = (sample_bbox3d[1] - y_min) / self.granularity
        l, w = sample_bbox3d[3:5] / self.granularity
        r = sample_bbox3d[6]

        x_star, x_end = int(- l / 2), int(l / 2)
        y_star, y_end = int(- w / 2), int(w / 2)
        box2d_matrix = torch.stack(
            torch.meshgrid(
                torch.arange(x_star, x_end), torch.arange(y_star, y_end), torch.arange(0, 1),
                indexing='xy'
            ), 
            dim=-1
        )[:, :, 0, :]
        box2d_matrix = box2d_matrix @ Quaternion(axis=[0, 0, 1], angle=r).rotation_matrix.T
        box2d_matrix = box2d_matrix[..., :2].view(-1, 2)
        box2d_matrix += torch.tensor([x, y]).view(1, 2)

        index_x = box2d_matrix[:, 0].long().clamp(0, unsample_matrix.shape[1] - 1)
        index_y = box2d_matrix[:, 1].long().clamp(0, unsample_matrix.shape[0] - 1)
        unsample_matrix[index_y, index_x] = 1
        
        return unsample_matrix

    def _unsample_matrix(self, points_sem, label_sem, sample_id, sample_bbox3d, gt_bbox):
        x_min, y_min, x_max, y_max = self.mix_range
        h = int((y_max - y_min) / self.granularity)
        w = int((x_max - x_min) / self.granularity)

        unsample_matrix = points_sem.new_zeros([h, w])

        unmix_keep = self.mix_id_map[sample_id][label_sem] != -1

        unmix_site_x, unmix_site_y = points_sem[unmix_keep][:, :2].unbind(-1)
        unmix_index_x = ((unmix_site_x - x_min) / self.granularity).floor().long().clamp(0, w - 1)
        unmix_index_y = ((unmix_site_y - y_min) / self.granularity).floor().long().clamp(0, h - 1)
        unsample_matrix[unmix_index_y, unmix_index_x] = 1.

        for _bbox in gt_bbox:
            unsample_matrix = self._unsample_matrix_bbox(unsample_matrix, _bbox) 

        # flooding unsample range to prevent collision
        if not self.collision:
            radius = np.linalg.norm(sample_bbox3d[3:5]) / 2
            unsample_matrix = self._flooding(unsample_matrix, radius)

        return unsample_matrix

    def _unsample_matrix_det(self, points_all, sample_id, sample_bbox3d):
        x_min, y_min, x_max, y_max = self.mix_range
        h = int((y_max - y_min) / self.granularity)
        w = int((x_max - x_min) / self.granularity)
        
        unsample_matrix = points_all.new_zeros([h, w])
        unmix_site_x, unmix_site_y = points_all[:, :2].unbind(-1)
        unmix_index_x = ((unmix_site_x - x_min) / self.granularity).floor().long().clamp(0, w - 1)
        unmix_index_y = ((unmix_site_y - y_min) / self.granularity).floor().long().clamp(0, h - 1)
        unsample_matrix[unmix_index_y, unmix_index_x] = 1.
    ...
    def _sample_site(self, sample_matrix_sem, unsample_matrix_sem, depth, unsample_matrix_det=None):
        if not self.mix_on_points:
            assert unsample_matrix_det is not None, 'unsample_matrix_det must be provided when mix_on_points is False.'
            nozero_keep = (sample_matrix_sem != 0)
            mean_height = sample_matrix_sem[nozero_keep].mean()
            nopoints_sample_matrix = mean_height * (1 - unsample_matrix_det) * (1 - unsample_matrix_sem)
        sample_matrix_sem = sample_matrix_sem * (1 - unsample_matrix_sem)

        x_min, y_min = self.mix_range[:2]

        depth_up = (depth + self.depth_allowance)
        depth_down = (depth - self.depth_allowance)
        sample_keep = (self.depth_matrix > depth_down) & (self.depth_matrix < depth_up)

        sample_index = (sample_matrix_sem * sample_keep).nonzero()

        if len(sample_index) == 0:
            if not self.mix_on_points:
                sample_index = (nopoints_sample_matrix * sample_keep).nonzero()
                if len(sample_index) == 0:
                    return None
                sample_matrix_sem += nopoints_sample_matrix
            else:
                return None
            
        sample_index = sample_index[torch.randperm(len(sample_index))[0]]
        sample_index_y, sample_index_x = sample_index[0], sample_index[1]

        sample_site_z = sample_matrix_sem[sample_index_y, sample_index_x]
        sample_site_x = sample_index_x * self.granularity + x_min + \
                        np.random.uniform(0., self.granularity, 1)
        sample_site_y = sample_index_y * self.granularity + y_min + \
                        np.random.uniform(0., self.granularity, 1)

        return sample_site_x, sample_site_y, sample_site_z

    def cutmix(self, points, gt_cates, gt_bbox, gt_sem, mask_sem):
        assert len(points) == len(mask_sem), 'points and mask_sem should have same length'
        assert len(gt_bbox) == len(gt_bbox), 'gt_bbox and gt_sem should have same length'

        sample_cls_list = []
        for sample_cls, max_sample, min_sample, min_points in zip(
            self.sample_cls, self.max_sample, self.min_sample, self.min_points
        ):
            sample_id = self.CONFIG[sample_cls]['id_sem']
            sample_cls_list.extend(
                [(sample_id, min_points)] * np.random.randint(min_sample, max_sample+1)
            )
        np.random.shuffle(sample_cls_list)

        for sample_id, min_points in sample_cls_list:

            points_sem = points[mask_sem]
            filter_sem = self._bev_filter(points_sem)

            sample_inst, sample_depth, sample_bbox3d = self._sample_inst(sample_id, min_points)
            if sample_inst.shape[0] == 0:
                continue

            sample_matrix = self.sample_matrix_buffer.get(sample_id, None)
            if sample_matrix is None:
                sample_matrix = self._sample_matrix(points_sem[filter_sem], gt_sem[filter_sem], sample_id)
                self.sample_matrix_buffer[sample_id] = sample_matrix
            
            unsample_matrix = self._unsample_matrix(points_sem[filter_sem], gt_sem[filter_sem], sample_id, sample_bbox3d, gt_bbox)
            unsample_matrix_det = None
            if not self.mix_on_points:
                filter_det = self._bev_filter(points)
                unsample_matrix_det = self._unsample_matrix_det(points[filter_det], sample_id, sample_bbox3d)

            sample_site = self._sample_site(sample_matrix, unsample_matrix, sample_depth, unsample_matrix_det)
            if sample_site == None:
                continue
            sample_site = sample_inst.new_tensor(sample_site)

            sample_inst[:, :3] += sample_site
            sample_bbox3d[:3] += sample_site
            
            # masks = box_np_ops.points_in_rbbox(points.numpy(), sample_bbox3d[None, :].numpy())
            # mask_points = np.logical_not(masks.any(-1))

            points = torch.cat([points, sample_inst])
            gt_cates = torch.cat([gt_cates, self.sem2det_map[sample_id:sample_id+1]])
            gt_bbox = torch.cat([gt_bbox, sample_bbox3d[None, :]])
            gt_sem = torch.cat([gt_sem, gt_sem.new_ones([sample_inst.shape[0]]) * sample_id])
            mask_sem = torch.cat([mask_sem, mask_sem.new_ones(sample_inst.shape[0], dtype=torch.bool)])

        self.sample_matrix_buffer.clear()

        return points, gt_cates, gt_bbox

    def __call__(self, input_dict):
        points = input_dict['points'].tensor  # 'torch.float32'
        gt_cates = torch.from_numpy(input_dict['gt_labels_3d']).long()  # 'torch.int64'
        gt_bbox = input_dict['gt_bboxes_3d'].tensor  # 'torch.float32'
        gt_sem = torch.from_numpy(input_dict['pts_semantic_mask']).long()  # 'torch.int64'        
        # mask_sem = points[:, -1] == 0
        mask_sem = points.new_zeros(points.shape[0], dtype=torch.bool)
        mask_sem[:gt_sem.shape[0]] = True  # 'torch.bool'
        
        points_cutmix, gt_cates_cutmix, gt_bbox_cutmix = self.cutmix(
            points, gt_cates, gt_bbox, gt_sem, mask_sem
        )

        input_dict['gt_bboxes_3d'] = input_dict['gt_bboxes_3d'].new_box(gt_bbox_cutmix.cpu().numpy())
        input_dict['gt_labels_3d'] = gt_cates_cutmix.cpu().numpy().astype(np.int64)
        input_dict['points'] = input_dict['points'].new_point(points_cutmix.cpu().numpy())
        if self.pop_sem:
            input_dict.pop('pts_semantic_mask')
